...
import com.xiaojinzi.lib.res.dto.NameOfAlipayBillTransferSuccess

@ServiceAnno(
    SubBillParseService::class,
    name = [NameOfAlipayBillTransferSuccess],
)
class AlipayBillTransferSuccessBillParseServiceImpl : SubBillParseService {

    override val sourceType: ParseSourceType = ParseSourceType.Alipay_Transfer_Success

    override fun parse(info: AccessibilityNodeInfo): BillParseResultDTO? {
        return try {
            val contentNode = info
                .findAccessibilityNodeInfosByText("转账成功")
                .first()
                .parent
