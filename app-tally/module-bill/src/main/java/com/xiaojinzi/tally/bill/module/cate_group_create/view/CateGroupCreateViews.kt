...
@Composable
private fun CateGroupCreateView() {
    val context = LocalContext.current
    val scope = rememberCoroutineScope()
    val vm: CateGroupCreateViewModel = viewModel()
        ...
        if (isUpdate) {
            ...
            Button(
                modifier = Modifier
                    .fillMaxWidth()
                    .nothing(),
                onClick = { vm.deleteCateGroup(context = context) },
...
fun CateGroupCreateViewWrap() {
    ...
    Scaffold(
        topBar = {
            AppbarNormal(
                titleRsd = if (isUpdate) R.string.res_str_update_category_group else R.string.res_str_new_category_group,
                menu1IconRsd = canNext.run {
                    if (canNext) {
                        if (isUpdate) {
                            R.drawable.res_right2
                        } else {
                            R.drawable.res_right2
                        }
                    } else {
                ...
                menu1ClickListener = {
                    vm.newCateGroup(context = context)
                }
            )
        }
    ) {
...
@Preview
@Composable
private fun CateGroupCreateViewPreview() {
    CateGroupCreateView()
}